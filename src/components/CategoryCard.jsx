...
export default function CategoryCard({ cat, d, index, slug }) {
  ...
  const previewImage = cat.items?.[0]?.image || cat.image;
  const shortSummary = (text) => {
    if (!text) return "";
    if (text.includes(".")) {
      return text.split(".")[0].trim() + ".";
    }
    return text; // fallback if no full stop
  };
  return (
    <motion.div
      ref={ref}
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {hasItems ? `${cat.items.length} product${cat.items.length !== 1 ? "s" : ""}` : `${cat.services?.length ?? 0} services`}
                </p>
              </div>
              <motion.div
          ...
          >
            ...
            >
              {shortSummary(cat.summary)}
            </p>

            {/* Preview pills — item names or service snippets */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
              {hasItems
                ...
                : cat.services?.slice(0, 3).map((svc, i) => (
                    <span
                      key={i}
                      style={{
                        padding: "2px 8px",
                    ...
                    >
                      {svc.split(" – ")[0].split(" ").slice(0, 3).join(" ")}
                    </span>
                  ))}
              {hasItems && cat.items.length > 3 && (
                <span
                  style={{
                    padding: "2px 8px",
                    borderRadius: 99,
